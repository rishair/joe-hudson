...
// E-053 — shared base of the coach system prompt (the v5b body).
//
// This is the SAME content as `web-app/app/lib/system-prompt.ts`'s
// SYSTEM_PROMPT (the deployed v5b/v1 prompt, lifted verbatim from
// coach-app/prompts/v1.md). It is factored out here so the four
// authority-spectrum variants (v5b / reflective / grounded / joe-voiced)
// can each compose the SAME safety floor + methodology body and differ ONLY
// in a stance layer appended at the end.
//
// WHY factor it instead of copy-pasting the prompt into four files:
//   - The experiment requires SAFETY FIRST and the anti-pattern guardrails to
//     stay intact in ALL four variants and the authority dial must NOT loosen
//     the safety floor. Sharing one base GUARANTEES that by construction — a
//     variant cannot accidentally drop or weaken a safety clause, because it
//     never owns its own copy of the safety section.
//   - A reviewer reading any one variant file sees a short, legible stance
//     layer (the only thing that differs) rather than re-reading 130 lines of
//     identical body to diff the change.
//
// `system-prompt.ts` is intentionally NOT modified. The `v5b` profile imports
// SYSTEM_PROMPT directly from there (it IS the deployed baseline / control),
// so the deployed default and the `v5b` selector option are the exact same
// bytes with no risk of drift. This base module is consumed only by the three
// NEW variants, which append their stance layer to it.
//
// If the v5b body is ever revised, update BOTH this file and system-prompt.ts
// (the two must stay in lockstep — they are the same prompt). A future
// refactor could have system-prompt.ts re-export BASE_PROMPT, but E-053's
// blast radius forbids touching the chat route's import surface, so the
// duplication is accepted and documented here.

export const BASE_PROMPT = `# Coach v1 — Hand-Authored AoA System Prompt
